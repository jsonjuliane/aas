...
## Local Wi-Fi hotspot (phone connects to Pi)

Use this when you want a phone/laptop to connect directly to the Pi without internet.
This is intended for the local setup/app/dashboard UI.

> Warning: if you are SSH'd into the Pi over Wi-Fi, enabling hotspot mode can disconnect you. Use Ethernet/keyboard if possible for first setup.
...
2. Join `SmartShell-Setup`.
3. Password: `smartshell123`.
4. Ignore "No Internet" warning.
5. Open local service URL, for example `http://192.168.4.1:8080` once the app/dashboard server exists.
...
```bash
sudo nmcli connection down smartshell-hotspot
```

Remove hotspot config:

```bash
sudo nmcli connection delete smartshell-hotspot
```

Custom SSID/password:

```bash
sudo bash deploy/setup-pi-hotspot.sh "SmartShell-Setup" "yourpassword"
```
